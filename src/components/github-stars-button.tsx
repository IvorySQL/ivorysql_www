...
export const GitHubStarsButton = () => {
  ...
  const kFormat = (num: number | null): string => {
    if (num === null) return "—";
  ...
  return (
    <a
      ...
      className={cn(
        "group flex items-center gap-1.5 rounded-md border-0 px-2.5 py-1.5 text-sm",
    ...
    >
      ...
      <span className="tabular-nums">{kFormat(stars)}</span>
    </a>
  );
};
