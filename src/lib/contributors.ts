const CONTRIBUTORS_BY_YEAR: Record<number, string[]> = {
  2026: [
    "123123213weqw",
    "3296442201",
    "Adria0615",
...
const CONTRIBUTOR_OVERRIDES: Record<string, ContributorOverride> = {
  ...
  faweizhao: {
    name: "Fawei Zhao",
    github: "faweizhao26",
    avatarSrc: "/images/contributors/fawei-zhao.jpeg",
  },
  ...
  yasirhussainshah: {
    name: "Yasir Hussain Shah",
    github: "yasir-hussain-shah",
  },
  ...
  zuinuan123: { name: "Sixian Gao", github: "zuinuan123" },
};

function normalizeContributorId(value: string): string {
  return value
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9]/g, "")
...
function looksLikeGithubHandle(value: string): boolean {
  return /^[a-z0-9-]+$/.test(value);
}

function slugBeautyScore(value: string): number {
  let score = 0;
  if (/[-_]/.test(value)) score += 2;
  if (/[A-Z]/.test(value.slice(1))) score += 2;
  if (!looksLikeGithubHandle(value)) score += 1;
  return score;
}

function humanizeContributorName(value: string): string {
...
export const contributors: Contributor[] = Array.from(contributorMap.values())
  .map((item) => {
...
export const contributorYears: number[] = Object.keys(CONTRIBUTORS_BY_YEAR)
  .map(Number)
  .sort((a, b) => b - a);
